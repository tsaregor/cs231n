...
def content_loss(content_weight, content_current, content_original):
    ...
    tmp=content_weight*((content_current-content_original)**2).sum()
    return tmp
...
def gram_matrix(features, normalize=True):
    ...
    dim=features.size()
    f_=features
 #   f_.resize_(dim[0],dim[1],dim[2]*dim[3])
    gram=torch.zeros((dim[0],dim[1],dim[1]),device='cuda')
    for c in range(gram.size()[1]):
      gram[:,c,:]=(f_[:,c,:,:]*f_[:,:,:,:]).sum(dim=3).sum(dim=2)
    if(normalize==True):
      gram=gram/(dim[1]*dim[2]*dim[3])
    return gram
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
...
def tv_loss(img, tv_weight):
    ...
    q1=((img[:,:,:,1:]-img[:,:,:,:-1])**2).sum()
    q2=((img[:,:,1:,:]-img[:,:,:-1,:])**2).sum()
    return (q1+q2)*tv_weight
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
