...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers-1):
          if self.normalization == "batchnorm":
            a,b=affine_norm_relu_forward(a, self.params['W'+str(i+1)], self.params['b'+str(i+1)],self.params['gamma'+str(i+1)], self.params['beta'+str(i+1)],self.bn_params[i])
          else:
            a,b=affine_relu_forward(a, self.params['W'+str(i+1)], self.params['b'+str(i+1)])
          cache.append(b)

          if self.use_dropout:
            a,b=dropout_forward(a,self.dropout_param)
            drop_cache.append(b)
        a,b=affine_forward(a,self.params['W'+str(self.num_layers)], self.params['b'+str(self.num_layers)])
        cache.append(b)
        scores=a
        # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        ############################################################################
        ...
        for i in reversed(range(self.num_layers-1)):
          ...
          if self.normalization == "batchnorm":
            dout, dw, db,dgamma, dbeta=affine_norm_relu_backward(dout,cache[i])
